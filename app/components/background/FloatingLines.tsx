...
const vec3 PINK  = vec3(0.91373, 0.27843, 0.96078); // Pre-divided
const vec3 BLUE  = vec3(0.18431, 0.29412, 0.63529); // Pre-divided

// Optimized rotation matrix (inline to reduce function calls)
mat2 rotate(float r) {
  float c = cos(r);
  float s = sin(r);
  return mat2(c, s, -s, c);
...
// Optimized wave function with reduced calculations
float wave(vec2 uv, float offset, vec2 screenUv, vec2 mouseUv, bool shouldBend) {
  ...
  if (shouldBend) {
    ...
    // Use faster approximation for exp when possible
  ...
  float m = uv.y - y;
  float absM = abs(m);
  return 0.0175 / max(absM + 0.01, 1e-3) + 0.01;
}
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  // Pre-calculate rotation for bottom waves
  if (enableBottom && bottomLineCount > 0) {
  ...
  // Pre-calculate rotation for middle waves
  if (enableMiddle && middleLineCount > 0) {
  ...
  // Pre-calculate rotation for top waves
  if (enableTop && topLineCount > 0) {
...
type FloatingLinesProps = {
  ...
  maxPixelRatio?: number; // New prop for performance control
  powerSaveMode?: boolean; // New prop to limit FPS
};
...
export default function FloatingLines({
  ...
  mixBlendMode = "screen",
  maxPixelRatio = 1.5, // Reduced from 2 for better performance
  powerSaveMode = false, // Limit to 30fps when true
}: FloatingLinesProps) {
  ...
  const gradientUniforms = useMemo(() => {
    ...
    return { gradientArray, gradientCount };
  }, [linesGradient]);

  // Memoize wave positions to prevent unnecessary scene recreation
  ...
  useEffect(() => {
    ...
    const uniforms = {
      ...
      topWavePosition: {
        value: wavePositions.top.clone(),
      },
      middleWavePosition: {
        value: wavePositions.middle.clone(),
      },
      bottomWavePosition: {
        value: wavePositions.bottom.clone(),
      },
      ...
      lineGradient: { value: gradientUniforms.gradientArray },
      lineGradientCount: { value: gradientUniforms.gradientCount },
    };
    ...
    const material = new ShaderMaterial({
      ...
      vertexShader,
      fragmentShader,
      precision: "mediump", // Force medium precision
    });
    ...
    const ro =
      typeof ResizeObserver !== "undefined"
        ? new ResizeObserver(throttledSetSize)
        : null;
    ...
    let raf = 0;
    const targetFPS = powerSaveMode ? 30 : 60;
    const frameInterval = 1000 / targetFPS;

    const renderLoop = (currentTime: number) => {
      raf = requestAnimationFrame(renderLoop);

      // Pause rendering when tab is hidden or component is not visible
